  ...
  if (element.textContent && element.textContent.trim()) {
    ...
    if (text.length < 50) {
      ...
      const xpathResult = document.evaluate(
        `//${element.nodeName.toLowerCase()}[contains(text(), "${text.replace(/"/g, '\\"')}")]`,
        document,
        null,
      ...
      if (xpathResult.snapshotLength === 1) {
        return `//${element.nodeName.toLowerCase()}[contains(text(), "${text.replace(/"/g, '\\"')}")]`
      }
    }
  }
...
function updateOverlayPanel(element) {
  ...
  const optimizedXpath = getOptimizedXPath(element)
...
function updateResultCount(count, isError = false, errorMessage = "") {
  ...
  if (isError) {
    ...
    resultCountElement.textContent = count === 0 ? "No matches" : count === 1 ? "1 match" : `${count} matches`
...
function handleMouseMove(event) {
  ...
  if (element && !element.closest("#xpath-overlay-panel")) {
